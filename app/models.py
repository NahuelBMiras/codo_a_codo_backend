...
class PathImg(db.Model):  # Renombrado de Path a PathImg
    ...
    producto_rel = db.relationship('Articulo', backref=db.backref('path_img', uselist=False))
...
class Products(db.Model):
    ...
    tiempo_de_subida = db.Column(db.DateTime, default=db.func.current_timestamp(), nullable=False)

    categoria_rel = db.relationship('Categoria', backref=db.backref('producto', lazy=True))
    stock_rel = db.relationship('Stock', backref=db.backref('producto', lazy=True))
    articulo_rel = db.relationship('Articulo', backref=db.backref('producto', lazy=True))
...
class Articulo(db.Model):
    ...
    id_path_img_producto = db.Column(db.Integer, db.ForeignKey('path_img.id_path_img_producto'), nullable=False)

    path_img_producto = db.relationship('PathImg', backref=db.backref('articulo', lazy=True))
    producto_rel = db.relationship('Products', backref=db.backref('articulo', uselist=False))
